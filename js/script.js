$(document).ready(function() {
  ...
  var authors = [ // niz svih autora citata
    "Nelson Mandela", // 0
    "Dr. Seuss", // 1
    "Dr. Seuss", // 2
    "Marilyn Monroe", // 3
  ...
  var elementPos; // promenjiva u kojoj se čuvaju pozicije elemenata iz nizova citata i autora

  // događaj klika dugmeta gde se smenjuju citati i odgovarajući autori citata
  $("#quoteButton").on("click", function() {
    $("#quoteButton").html("Next quote"); // promena natpisa na dugmetu nakon prvog klika
    $("#logo").css("visibility", "visible"); // pojavljivanje dugmeta Twittera nakon prvog klika

    if(quotes.length === 0) { // proveravanje da li je niz citata prazan, ako jeste prikazuje se konačna poruka korisniku
      $("#quote").html("You read all quotes!");
      $("#author").html("");
      $("#logo").css("visibility", "hidden"); // sakrivanje dugmeta Twittera nakon prolaska kroz sve citate
      $("#quoteButton").html("Read again"); // omogućavanje korisnicima da klikom na dugme ponovo mogu da čitaju citate

      // ako korisnik klikne na dugme web strana se osveži
      $("#quoteButton").on("click", function() {
        history.go(0);
      });
    }
    else { // ako niz citata nije prazan, prikaži sledeći citat
      elementPos = Math.floor(Math.random() * authors.length); // dodeljivanje nasumične vrednosti promenjivoj elementPos

      $("#quote").html("<q>" + quotes[elementPos] + "</q>"); // promena citata
      $("#author").html(authors[elementPos]); // promena autora

      quotes.splice(elementPos, 1); // izbacivanje već prikazanog citata iz niza citata
      authors.splice(elementPos, 1); // izbacivanje već prikazanog autora iz niza autora
    }
  });

  // omogućavanje postavljanja citata na Twitter društvenu mrežu
